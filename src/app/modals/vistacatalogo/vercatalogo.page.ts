...
export class VistaCatalogoPage  {
  ...
  constructor( private modalController: ModalController, public catalogoService: CatprovService,
              ...
              private callNumber: CallNumber, private photoLibrary: PhotoLibrary) {
                      ...
                      //   console.log(this.parametro.get('rutacatalogo'));
                         ...
                         this.totalcart = this.parametro.get('totalcarrito');
...
async decoratePhoneNumber(PhoneNumber: any) {
...
async ponerencarrito() {

     this.storage.get('TotalCart').then((val) => {
      this.totalcart = val;
    });

    this.storage.set('TotalCart', this.totalcart + 1).then((response) => {
...
/* async downloadFile(archivo:any, nombrearchivo : any) {
  await this.fileTransfer.download(archivo, this.file.externalRootDirectory +
...
async downloadFile(archivo: any, nombrearchivo: any) {
  ...
  async downloadFile2carrete(archivo: any, nombrearchivo: any) {
   // this.fileTransfer.download(archivo, path + filename, true).then((entry) =>
   console.log(archivo, nombrearchivo);
    this.http.get(archivo, { responseType: 'blob' })
      .subscribe((imageBlob: Blob) => {
        // imageBlob is the binary data of the the image
...
whatsappShare(a1: any) {
  ...
  // guardar imagenes al favorito funciones
